...
test("expands a file region and records the include capability", async () => {
	const result = await render(
		"<!-- @include: tests/fixtures/markdown/include-example.md#public-api -->",
	);
	assert.match(
		result.code,
...
test("supports inclusive line ranges and open bounds", () => {
	const source =
		"<!-- @include: tests/fixtures/markdown/include-example.md{2-4} -->";
	...
	const open = expandMarkdownIncludes(
		"<!-- @include: tests/fixtures/markdown/include-example.md{-2} -->",
	);
...
test("keeps fenced, invalid, missing, and recursive includes literal", () => {
	const source = [
		"```markdown",
		"<!-- @include: tests/fixtures/markdown/include-example.md -->",
		"```",
		"",
		"<!-- @include: tests/fixtures/markdown/missing.md -->",
		"<!-- @include: tests/fixtures/markdown/include-example.md{0-2} -->",
	].join("\n");
	const expanded = expandMarkdownIncludes(source);
